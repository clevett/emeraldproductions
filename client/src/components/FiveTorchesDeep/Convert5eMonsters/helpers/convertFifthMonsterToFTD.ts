...
const convertFifthMonstersToFTD = (monster: FifthEditionMonster): MonsterFTD => {
  ...
  const resistances = damage_resistances ? joinArrayIntoList(damage_resistances) : null

  const speed = monster.speed ? convertSpeed(monster.speed) : null

  const vulnerabilities = damage_vulnerabilities ? joinArrayIntoList(damage_vulnerabilities) : null
