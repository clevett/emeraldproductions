...
const BeastTableRow = ({ beastData, beastButton, buttonType }) =>  {
  const buttonDetermination = () => {
    if (buttonType === 'add') {
      return <button className='btn btn-success btn-rounded' onClick={() => beastButton(beastData, buttonType)}><span>+</span></button>
    } else {
      return <button className='btn btn-danger btn-rounded' onClick={() => beastButton(beastData, buttonType)}><span>-</span></button>
    }
  }
