...
class TravelTool extends React.Component {
  ...
  onValueChange = async (key, value) => {
    ...
    if (key.includes("miles") || key === "terrain" || key === "weather") {
      const adjustedMilesPerHour = adjustedSpeed(
        this.state.milesPerHour,
        this.state.multiplier
      );
      await this.setState({
        adjustedMilesPerHour,
        adjustedMilesPerDay: adjustedSpeed(
          this.state.milesPerDay,
          this.state.multiplier
        ),
        travelTime: determineTravelTime(
          adjustedMilesPerHour,
          this.state.milesToTravel
        ),
      });
    }
  };
        ...
        <Row className="content mb-5 w-100">
          <CardGroup className="mb-3 w-100">
            <DisplayCard
              ...
              result={this.state.adjustedMilesPerHour}
            />
            <DisplayCard
              title="Miles per Day"
              result={this.state.adjustedMilesPerDay}
            />
            <DisplayCard
              title={`Time to Travel ${this.state.milesToTravel} Miles`}
              result={this.state.travelTime}
            />
          </CardGroup>
