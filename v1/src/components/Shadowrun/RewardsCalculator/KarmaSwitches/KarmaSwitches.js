...
const KarmaSwitches = ({ updateState, nuyenSituationModifier }) => {
  ...
  const updateKarmaTotal = () => {
    updateState(
      "karmaModifier",
      parseInt(surviveBonus + objectiveBonus + situationBonus)
    );
  };

  useEffect(updateKarmaTotal, [surviveBonus, objectiveBonus, situationBonus]);

  const germanToggle = (name, status) =>
    status ? setSituationBonus(nuyenSituationModifier) : setSituationBonus(0);
  const surviveToggle = (name, status) =>
    status ? setSurviveBonus(2) : setSurviveBonus(0);
  const sliderChange = (event) =>
    setObjectiveBonus(parseInt(event.target.value));
      ...
      <Row>
        ...
        <Col>
          ...
          <Row className="slider justify-content-center">
            <label className="sr-only" htmlFor="completedObjectives">
              Choose objectives completed
            </label>
            <input
              type="range"
